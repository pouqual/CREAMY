...
from utils.loss import TripletLoss, CCL, contra_loss
from dataset import get_loader
from config import get_args
from models_ing import get_model
...
def trainIter(args, split, loader, model, optimizer,
              loss_function, contra_loss, scaler, metrics_epoch):
    with autocast():
        ...
        if split == 'val':
            with torch.no_grad():
                img_feat, recipe_feat, proj_feats, ing_feat, = model(img, title, ingrs, instrs)
        else:
            out = model(img, title, ingrs, instrs,
                        freeze_backbone=args.freeze_backbone)

            img_feat, recipe_feat, proj_feats, ing_feat = out

        loss_recipe, loss_paired, loss1 = 0, 0, 0
        ...
        if img is not None:
            ...
            loss1 = contra_loss(img_feat, ing_feat)

            loss_paired = loss_function(img_feat, recipe_feat)

            metrics_epoch['loss_paired'].append(loss_paired.item())

        loss = loss_paired + args.recipe_loss_weight*loss_recipe + 0.001 * loss1
    ...
    ing_feat = ing_feat.cpu().detach().numpy()

    return img_feat, recipe_feat, metrics_epoch, ing_feat
    ...
    for epoch in range(curr_epoch, args.n_epochs):
        ...
        for split in ['train', 'val']:
            ...
            for i in range(total_step*emult):
                ...
                optimizer.zero_grad()
                model.zero_grad()
                img_feat, recipe_feat, metrics_epoch, ing_feat = trainIter(args, split,
                                                                 this_iter_loader,
                                                                 model, optimizer,
                                                                 loss_function,
                                                                 contra_loss,
